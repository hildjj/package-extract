...
program
  ...
  .option('-o, --output <filename>', 'name of output file, relative to package.json', 'package.js')
...
async function findRoot() {
  ...
  while (dir) {
    ...
    try {
      const f = path.join(dir, opts.package)
      await fs.access(f)
      return f
    } catch (ignored) {
      prev.add(dir)
  ...
  throw new Error(`No ${opts.package} file found starting from ${init}`)
}

async function main() {
  const pkg = await findRoot()
  const props = JSON.parse(await fs.readFile(pkg))
  let out = '// Generated by package-extract.  Do not modify by hand.\n\n'
